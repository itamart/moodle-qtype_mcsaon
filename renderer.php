...
/**
 * Subclass for getting the bits of output specific from the standard multichoice multi
 *
 * @copyright  2012 Itamar Tzadok
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qtype_mcsaon_multi_renderer extends qtype_multichoice_multi_renderer {
    public function formulation_and_controls(question_attempt $qa,
        ...
        foreach ($question->get_order($qa) as $value => $ansid) {
            ...
            if ($options->rightanswer) {
                $feedbackimg[] = $this->feedback_image($this->is_right($ans), $isselected);
                if ($isselected) {
                      $class .= ' ' . $this->feedback_class($this->is_right($ans));

                }
            } else if ($options->correctness && $isselected) {
